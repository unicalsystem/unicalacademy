...
if ( ! class_exists( 'LP_Shortcode_Zoom_Meeting' ) ) {

	/**
	 * Class LP_Shortcode_Zoom_Meeting
	 */
	class LP_Shortcode_Zoom_Meeting extends LP_Abstract_Shortcode {

		/**
		 * LP_Shortcode_Zoom_Meeting constructor.
		 *
		 * @param  mixed $atts
		 */
		...
		public function output() {
			...
			try {
				...
				$data = ! empty( $results['live_value'] ) ? (array) json_decode( $results['live_value'] ) : array();

				LP_Addon_Live_Preload::$addon->get_template(
					'shortcode/single-zoom.php',
					array(
						'data' => $data,
					)
				);
