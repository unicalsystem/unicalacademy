...
//                $timer_row = "";
    ...
    /*******************************************************************************************************/
        ...
        if($enable_box_shadow){
            $quiz_styles .=  "box-shadow: 0 0 15px 1px " . Quiz_Maker_Data::hex2rgba($box_shadow_color, '0.4') . ";";
        }else{
    ...
    /*******************************************************************************************************/
        ...
        $quiz_container_first_part = "
            <div class='ays-quiz-container ".$quiz_theme." ".$custom_class."'
...
//                                'score' => $correct_answered_count_cat[$cat_id] . " / " . $quiz_weight_cats[$cat_id],
                ...
                $not_answered = 0;
                foreach ($user_answered as $answered_question_ids => $u_answer) {
                    $user_question_id = explode('_', $answered_question_ids)[2];
                    $is_infliance = Quiz_Maker_Data::is_question_not_influence($user_question_id);
                    if(!$is_infliance  && $u_answer == ''){
                        $not_answered++;
                    }
                }
                ...
                $message_data = array(
                    ...
                    'user_pass_time' => Quiz_Maker_Data::get_time_difference(esc_sql($_REQUEST['start_date']), esc_sql($_REQUEST['end_date'])),
                ...
                if(isset($options->email_config_replyto_email) && $options->email_config_replyto_email != "") {
                    if(filter_var($options->email_config_replyto_email, FILTER_VALIDATE_EMAIL)){
                        ...
                        $nreply_smtp = stripslashes($options->email_config_replyto_email);
                    }
                }
                ...
                // $use_smtp = isset($options->enable_smtp) && $options->enable_smtp == "on" ? true : false;
                $use_smtp = false;
                ...
                if ($send_mail_to_user) {
                    if (isset($_REQUEST['ays_user_email']) && filter_var($_REQUEST['ays_user_email'], FILTER_VALIDATE_EMAIL)) {
                        ...
                        // if ($use_smtp) {
                            ...
                            if($nreply_smtp != ""){
                                $mail->addReplyTo($nreply_smtp, $replyto_name);
                            }
                            ...
                            if ($cert && $final_score >= intval($options->certificate_pass)) {
                                if($pdf_content === true){
                                    $mail->addAttachment( __DIR__ . '/certificate.pdf', 'Certificate.pdf', 'base64', 'application/pdf');
                                }
                                $sendMail = false;
                ...
                if ($send_mail_to_admin) {
                    if (filter_var(get_option('admin_email'), FILTER_VALIDATE_EMAIL)) {
                        ...
                        // if ($use_smtp) {
                            ...
                            //Server settings
                            $admin_mail->SMTPDebug = false;
                            $admin_mail->CharSet = 'UTF-8';

                            $admin_mail->isSMTP();
                            $admin_mail->Host = isset($options->smtp_host) ? $options->smtp_host : '';
                            $admin_mail->SMTPAuth = true;
                            $admin_mail->Username = isset($options->smtp_username) ? $options->smtp_username : '';
                            $admin_mail->Password = isset($options->smtp_password) ? $options->smtp_password : '';
                            $admin_mail->SMTPSecure = isset($options->smtp_secure) ? $options->smtp_secure : '';
                            $admin_mail->Port = isset($options->smtp_port) ? $options->smtp_port : '';

                            $admin_mail->SetFrom(isset($options->smtp_username) ? $options->smtp_username : '', $uname);

                            if ($send_mail_to_site_admin) {
                                $admin_mail->addAddress(get_option('admin_email'), '');
                            }
                            ...
                            if(isset($options->additional_emails) && !empty($options->additional_emails)) { 
                                ...
                                foreach($additional_emails as $additional_email){
                                    if(filter_var(trim($additional_email), FILTER_VALIDATE_EMAIL)){
                                        $admin_mail->addAddress(trim($additional_email), '');
                                    }
                                }
                            }
                            if($nreply_smtp != ""){
                                $admin_mail->addReplyTo($nreply_smtp, $replyto_name);
                            }
                            //$mail->addReplyTo('noreply@example.com', 'Don\'t reply');
                            $admin_mail->isHTML(true);
                            $admin_mail->Subject = stripslashes($quiz['title']).$admin_subject;
                            $admin_mail->Body = $message_content;
                            if($send_certificate_to_admin){
                                if ($cert && $final_score >= intval($options->certificate_pass)) {
                                    if($pdf_content === true){
                                        $admin_mail->addAttachment( __DIR__ . '/certificate.pdf', 'Certificate.pdf', 'base64', 'application/pdf');
                                    }
                                    if($admin_mail->send()){
                ...
                if ($result) {
                    ...
                    if ($woo && isset($product_id)) {
                        ...
                        $product = array(
                            ...
                            'link'  => "<a href=\"?add-to-cart=$product_id\" data-quantity=\"1\" class=\"action-button product_type_simple add_to_cart_button ajax_add_to_cart\" data-product_id=\"$product_id\" data-product_sku=\"\" aria-label=\"$cart_text\" rel=\"nofollow\">$cart_text</a>"
                        );
                    }
    ...
    public function ays_rate_the_quiz(){
        ...
        if ($results >= 0 && Quiz_Maker_Data::ays_set_rate_id_of_result($rate_id)) {
            ob_end_clean();
            $ob_get_clean = ob_get_clean();
            echo json_encode(array(
                'quiz_id'   => $quiz_id,
    ...
    public function ays_quiz_check_user_started() {
        ...
        if($user_id == 0){
            $started_user_count = Quiz_Maker_Data::get_limit_user_count_by_ip($quiz_id);
        }else{
            $started_user_count = Quiz_Maker_Data::get_limit_user_count_by_id($quiz_id, $user_id);
        }
    ...
    public function ays_add_google_sheets($data) {
        ...
        if(isset($reports) && !empty($reports)){
            ...
            $start_date = isset($reports['start_date']) && $reports['start_date'] != '' ? $reports['start_date'] : '';
            $end_date   = isset($reports['end_date']) && $reports['end_date'] != '' ? $reports['end_date'] : '';
            $score      = isset($reports['score']) && $reports['score'] != '' ? $reports['score']."%" : '';
            $points     = isset($reports['points']) && $reports['points'] != '' ? $reports['points'] : '';
            $duration   = isset($reports['duration']) && $reports['duration'] != '' ? $reports['duration']."s" : '';
        ...
        $name  = isset($_POST['ays_user_name'])  && $_POST['ays_user_name']  != '' ? $_POST['ays_user_name']  : '';
        $email = isset($_POST['ays_user_email']) && $_POST['ays_user_email'] != '' ? $_POST['ays_user_email'] : '';
        $phone = isset($_POST['ays_user_phone']) && $_POST['ays_user_phone'] != '' ? $_POST['ays_user_phone'] : '';
