...
class Quiz_Maker_Admin
{
    ...
    public function ays_questions_statistics_export() {
        ...
        $sql = "SELECT question_ids FROM {$quizes_table} WHERE id = {$quiz_id} AND `status` = 'finished';";
        ...
        if( ! current_user_can( 'manage_options' ) ){
            $user_id_sql = "SELECT rp.user_id 
                            ...
                            LEFT JOIN {$wpdb->prefix}aysquiz_quizes AS qz
                                ON rp.quiz_id = qz.id
                            WHERE rep.status = 'finished' AND rp.status = 'finished' AND qz.author_id = ".$current_user;
    ...
    public function ays_results_export_csv($results, $attributes){
        ...
        global $wpdb;
        error_reporting(0);
        $export_file_fields = array('user','user_ip','quiz_name','start_date','end_date','score','points','duration','rate','review','name','email','phone');
            ...
            foreach ($options->correctness as $key => $option) {
                if (strpos($key, 'question_id_') !== false) {
                    ...
                    if($calc_method == 'by_correctness'){
                        ...
                        $row .= '<tr class="'.$tr_class.'">
                            <td>'.__('Question',$this->plugin_name).' ' . $index . ' :<br/>' . stripslashes($question["question"]) . '</td>
                            <td>'.__('User answered',$this->plugin_name).':<br/><p class="'.$ans_point_class.'">' . htmlentities(stripslashes($user_answered)) . '<br>'.$user_answered_images.'</p></td>
...
//            echo "cURL Error #:" . $err;
    ...
    public function ays_show_filters(){
        ...
        if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'ays_show_filters') {
            ...
            $sql = "SELECT COUNT(*) FROM {$wpdb->prefix}aysquiz_reports WHERE `status` = 'finished' ORDER BY id DESC";
            if( ! current_user_can( 'manage_options' ) ){
                $sql = "SELECT COUNT(*) 
                        FROM {$results_table} AS r
                        LEFT JOIN {$quiz_table} AS q
            ...
            if (isset($_REQUEST['flag']) && $_REQUEST['flag']) {
                $quiz_id_sql = "SELECT quiz_id FROM {$results_table}";
                if( ! current_user_can( 'manage_options' ) ){
                ...
                $quiz_id = (isset($_REQUEST['quiz_id']) && $_REQUEST['quiz_id'] != null) ? intval($_REQUEST['quiz_id']) : $quiz_id_sql;
                $sql = "SELECT COUNT(*) FROM {$wpdb->prefix}aysquiz_reports WHERE quiz_id IN ($quiz_id) AND `status` = 'finished' ORDER BY id DESC";
                if( ! current_user_can( 'manage_options' ) ){
                    $sql = "SELECT COUNT(*) 
                            FROM {$results_table} AS r
                            LEFT JOIN {$quiz_table} AS q
    ...
    public function ays_generate_cert_preview(){
        ...
        if($pdf_content){
            $cert_url = AYS_QUIZ_PUBLIC_URL . "/certificate.pdf";
